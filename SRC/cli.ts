  ...
  try {
    switch (cmd) {
      ...
      case 'mp:list-items': {
        ...
        const items = filterPlanItems(mp, { project_id, status: status as any, category: category as any, priority: priority as any, due_before, due_after });
      ...
      case 'mp:create-item': {
        ...
        const description = flags['description'] || '';
        const category = flags['category'] as any;
        const priority = flags['priority'] as any;
        ...
        if (!project_id || !title || !category || !priority) { usage(); process.exit(1); }
      ...
      case 'mp:update-item': {
        ...
        if (flags['category'] !== undefined) changes.category = flags['category'];
        if (flags['status'] !== undefined) changes.status = flags['status'];
        if (flags['priority'] !== undefined) changes.priority = flags['priority'];
        if (flags['due'] !== undefined) changes.due_date = flags['due'] === 'null' ? null : flags['due'];
        ...
        if (flags['replace-checklist'] !== undefined) {
          ...
          changes.checklist = labels as any;
        }
        const updated = updatePlanItem(mp, id, changes);
        if (!updated) { console.error('PlanItem not found'); process.exit(1); }
      ...
      case 'mp:toggle-check': {
        ...
        if (!itemId || !checkId || (checkedStr !== 'true' && checkedStr !== 'false')) { usage(); process.exit(1); }
        const ok = toggleChecklistItem(mp, itemId, checkId, checkedStr === 'true');
        if (!ok) { console.error('Toggle failed: item or checklist id not found'); process.exit(1); }
