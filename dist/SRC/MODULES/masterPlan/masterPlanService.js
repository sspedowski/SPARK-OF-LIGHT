...
export function createEmptyData(idGen, clock) {
    return { projects: [], planItems: [], uuid: idGen, now: clock };
}
export function createProject(data, input) {
    const project = {
        id: data.uuid(),
    ...
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'Project', change_type: 'Create', entity_id: project.id, after: project });
    return project;
}
export function updateProject(data, id, changes) {
    const p = data.projects.find(p => p.id === id);
    if (!p)
        return null;
    const before = { ...p };
    if (changes.name !== undefined)
        p.name = changes.name.trim();
    if (changes.description !== undefined)
        p.description = changes.description.trim();
    if (changes.status !== undefined)
        p.status = changes.status;
    if (changes.start_date !== undefined)
        p.start_date = changes.start_date;
    if (changes.target_end_date !== undefined)
        p.target_end_date = changes.target_end_date;
    if (changes.color !== undefined)
        p.color = changes.color;
    p.updated_at = data.now();
    // In a future audit log, record before & after diff.
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'Project', change_type: 'Update', entity_id: p.id, before, after: p });
    return p;
}
...
export function createPlanItem(data, input) {
    // Validate project exists
    if (!data.projects.some(p => p.id === input.project_id))
    ...
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'PlanItem', change_type: 'Create', entity_id: planItem.id, project_id: planItem.project_id, after: planItem });
    return planItem;
}
export function updatePlanItem(data, id, changes) {
    const pi = data.planItems.find(pi => pi.id === id);
    if (!pi)
        return null;
    const before = { ...pi, checklist: pi.checklist.map(c => ({ ...c })) };
    if (changes.title !== undefined)
        pi.title = changes.title.trim();
    if (changes.description !== undefined)
        pi.description = changes.description.trim();
    if (changes.category !== undefined)
        pi.category = changes.category;
    if (changes.status !== undefined)
        pi.status = changes.status;
    if (changes.due_date !== undefined)
        pi.due_date = changes.due_date;
    if (changes.priority !== undefined)
        pi.priority = changes.priority;
    if (changes.notes !== undefined)
        pi.notes = changes.notes.trim();
    ...
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'PlanItem', change_type: 'Update', entity_id: pi.id, project_id: pi.project_id, before, after: pi });
    return pi;
}
export function deletePlanItem(data, id) {
    ...
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'PlanItem', change_type: 'Delete', entity_id: id, project_id: before.project_id, before });
    return true;
}
export function filterPlanItems(data, filter) {
    return data.planItems.filter(pi => {
        if (filter.project_id && pi.project_id !== filter.project_id)
            return false;
        if (filter.status && !filter.status.includes(pi.status))
            return false;
        if (filter.category && !filter.category.includes(pi.category))
            return false;
        if (filter.priority && !filter.priority.includes(pi.priority))
            return false;
        if (filter.due_before && pi.due_date && pi.due_date > filter.due_before)
            return false;
        if (filter.due_after && pi.due_date && pi.due_date < filter.due_after)
            return false;
        return true;
    });
}
export function projectProgress(data, project_id) {
...
export function preloadMdcrTemplate(data, project_id, start_date) {
    ...
    for (const spec of MDCR_TEMPLATE) {
        ...
        created.push(createPlanItem(data, {
            project_id,
            title: spec.title,
...
// ─────────────────────────────────────────────────────────────
export function toggleChecklistItem(data, planItemId, checklistItemId, checked) {
    ...
    if (data.persist)
        void data.persist();
    if (data.audit)
        void data.audit({ entity_type: 'PlanItem', change_type: 'ChecklistToggle', entity_id: pi.id, project_id: pi.project_id, before: beforeItem, after: item, detail: 'Checklist item toggled' });
    return true;
}
// ─────────────────────────────────────────────────────────────
// Exhaustive status guard example
// ─────────────────────────────────────────────────────────────
export function isTerminalPlanItemStatus(status) {
    switch (status) {
        case 'NotStarted':
        case 'InProgress':
            return false;
        ...
        case 'Dropped':
            return true;
        default: {
            const _exhaustive = status; // compile-time exhaustiveness
            return _exhaustive;
        }
    }
}
